...
summary: "Un aperçu rapide de la configuration du projet et de la stack technologique que j'ai utilisée."
tags:
  - Python
  - CI/CD
  - Linter
...
## Pourquoi ai-je démarrer ce projet?
...
<Image
  width={600}
  height={400}
  src="/blog/render-deployment/github-actions.png"
/>

## Stack technologique

Ce projet utilise FastAPI en Python 3.0. Il fait le lien entre les webhook de Render et les webhook de l'application Github afin de récupérer et transmettre le status de déploiement d'une application render dans la branch github.
...
<Image
  width={600}
  height={400}
  src="/blog/render-deployment/stack.png"
/>
...
Si la `pull_request` est `merged`, nous allons maintenant aller récupérer le status de déploiement sur l'API de Render afin de voir si elle est `success` ou `failure` 
...
### 3. Assignation des status 📨

Je fais un mapping des status de Render vers les status de Github pour ensuite retourner à Github le status final de déploiement
...
### 4. Retour du status de déploiement à Github 📊

Une fois que nous avons le status final, nous pouvons renvoyer le status à Github
...
Alors ai-je atteint mon objectif initial ? Oui, le manque d'une application CI/CD de Render pour les déploiements est résolue avec mon application.
